...
│   │   │   ├── uploadArchive.ts       # Exports `uploadArchive` which orchestrates server-side upload: uploads processed archive to storage (`uploadArchiveToStorage`) and inserts data into temporary DB tables (`insertArchiveInTempTables`), providing progress updates.
