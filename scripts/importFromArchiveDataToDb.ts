...
(async function execute() {
  ...
  async function Upsert_Skeleton<T extends TableNames>(
    ...
    let cont = 0;
    console.log("Upsert_skeleton for ", table, items.length, "with batch size", batchSize);
    ...
    let query = supabase
      ...
      if(idColumn.length > 0){
        options = { ignoreDuplicates: true, onConflict: idColumn.join(',') }
      }
      ...
      if (error) {
        console.error(`Error inserting batch into ${table}:`, JSON.stringify(error));
        console.error(`Error inserting batch into ${table}:`, JSON.stringify(data));
  ...
  async function Upsert_Account() {
    const account = await getDataFromUnprocessedFile(GLOBAL_ARCHIVE_PATH, FilesToProcess.Account);

    let getItem = (item: any) => {
      const { account_id, created_via, username, created_at, account_display_name } = item;
  ...
  async function Upsert_Tweet_Media() {
    ...
    let getItem = (item: any) => {
      const { media_id, tweet_id, media_url, media_type, width, height, archive_upload_id } = item;
  ...
  async function Upsert_Liked_Tweets() {
    ...
    await Upsert_Skeleton(liked_tweets, "liked_tweets", getItem, 250, ["tweet_id"]);
  }
  ...
  async function Upsert_User_Mentions() {
    ...
    await Upsert_Skeleton(usersmentioned, "mentioned_users", getItemMentionedUser, 250, ["user_id"]);
    ...
    await Upsert_Skeleton(usersmentions, "user_mentions", getItem, 150, ["mentioned_user_id","tweet_id"]);
  }
  async function Upsert_Likes() {
    const likes = await getDataFromUnprocessedFile(GLOBAL_ARCHIVE_PATH, FilesToProcess.Likes);

    let getItem = (item: any) => {
      
      const {account_id,liked_tweet_id,archive_upload_id } = item;
      const newItem: InsertLikes = {account_id,liked_tweet_id,archive_upload_id};
      return newItem;
    };
    ...
    const processedLikes = likes.filter((item) => !tweetsToFilter.includes(item.liked_tweet_id));

    console.log("processedLikes", processedLikes.length);
    await Upsert_Skeleton(processedLikes, "likes", getItem, 250, ["liked_tweet_id","account_id"]);
  }
  async function Upsert_Followers() {
    ...
    await Upsert_Skeleton(followers, "followers", getItem, 250, ["follower_account_id","account_id"]);
  }
  ...
  async function Upsert_Following() {
    ...
    await Upsert_Skeleton(following, "following", getItem, 250, ["account_id","following_account_id"]);
  }
  ...
  async function Upsert_Tweet_Urls() {
    const urls = await getDataFromUnprocessedFile(GLOBAL_ARCHIVE_PATH, FilesToProcess.TweetUrls);
  ...
  async function importTweets() {
    ...
    await Upsert_Skeleton(communityarchive_tweets, "tweets", getItem, 250, ["tweet_id"]);
