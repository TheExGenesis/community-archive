...
for insert
to authenticated
with check ((bucket_id = 'archives'::text) AND (((( SELECT auth.jwt() AS jwt) -> 'app_metadata'::text) ->> 'user_name'::text) = (storage.foldername(name))[1]));

create policy "Allow authenticated archive deletes"
on storage.objects
for delete
to authenticated
using ((bucket_id = 'archives'::text) AND (((( SELECT auth.jwt() AS jwt) -> 'app_metadata'::text) ->> 'user_name'::text) = (storage.foldername(name))[1]));
