...
import { hasJevFilters, parseJevFilters } from '@/lib/bulletin/curation'
...
export async function GET(request: Request) {
  ...
  if (hasJevFilters(jev) && !(await isBulletinAdmin()))
    return NextResponse.json(
      { error: 'Admin filters only' },
      { status: 403, headers },
    )
  try {
    return NextResponse.json(
