...
DROP FUNCTION IF EXISTS public.refresh_account_activity_summary;
DROP MATERIALIZED VIEW IF EXISTS account_activity_summary;
...
SELECT
  ...
  (SELECT json_agg(row_to_json(l)) FROM (
    SELECT * FROM get_account_most_liked_tweets_archive_users(a.username, 5)
  ) l) AS most_liked_tweets_by_archive_users,
  (SELECT json_agg(row_to_json(r)) FROM (
    SELECT * FROM get_account_most_replied_tweets_by_archive_users(a.username, 5)
  ) r) AS most_replied_tweets_by_archive_users,
  (SELECT json_agg(row_to_json(rt)) FROM (
    SELECT * FROM get_account_top_retweet_count_tweets(a.username, 5)
  ) rt) AS most_retweeted_tweets,
  (SELECT json_agg(row_to_json(f)) FROM (
    SELECT * FROM get_account_top_favorite_count_tweets(a.username, 5)
