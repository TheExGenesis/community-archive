-- Fix functions to use correct table (public.temporary_data) for streaming stats
-- temporary_data tracks when tweets were actually streamed with inserted timestamp

-- Drop existing incorrect functions
...
-- Function for hourly stats from temporary_data (API streaming data)
...
BEGIN
    ...
    stats AS (
        ...
        WHERE td.inserted >= p_start_date 
          ...
          AND td.type = 'api_tweet'  -- Only API streamed tweets
