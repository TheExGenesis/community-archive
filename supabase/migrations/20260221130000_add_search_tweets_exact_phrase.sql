-- Add search_tweets_exact_phrase RPC function.
-- Uses ILIKE for exact substring matching (handles stop-word-heavy phrases
-- that FTS drops). Has 5min statement timeout vs PostgREST's 8s default.
-- The pg_trgm GIN index on tweets.full_text makes the ILIKE fast.
...
BEGIN
    ...
    WITH matching_tweets AS (
        ...
        WHERE t.full_text ILIKE '%' || exact_phrase || '%'
...
GRANT EXECUTE ON FUNCTION "public"."search_tweets_exact_phrase"("text", "text", "text", "date", "date", integer, integer) TO anon;
GRANT EXECUTE ON FUNCTION "public"."search_tweets_exact_phrase"("text", "text", "text", "date", "date", integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION "public"."search_tweets_exact_phrase"("text", "text", "text", "date", "date", integer, integer) TO service_role;
