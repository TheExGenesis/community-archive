-- Fix: delete_user_archive fails with FK violation on tweet_media_archive_upload_id_fkey
-- The function was only deleting tweet_media by tweet_id, but tweet_media also has
-- an archive_upload_id FK. Orphaned tweet_media rows (referencing the archive_upload
-- but not matching the tweet_id filter) blocked deletion of archive_upload rows.
-- Fix: add an explicit DELETE FROM tweet_media WHERE archive_upload_id = ANY($1)
-- before deleting archive_upload rows.
